...
def split_idiom_data( idiom_data, compositionality, language ) :
    ...
    ## Filter compositionality by idioms we have
    ##    In general the idioms in that dataset and this one are the same.
    ##     However, some idioms with hyphans have had their hyphans removed (to do the word split)
    ##     Since this sometimes changes meaning, we ignore those compositionalities. 
    new_compositionality = dict()
    for comp_idiom in compositionality.keys() :
        if comp_idiom in all_idioms :
    ...
    assert len( keep_idioms ) == 50
    print( "Set aside 50 idioms.", flush=True )
    ...
    print( "Have 20 in test, dev, train_from_test and train_from_dev", flush=True )
